...
where $\Omega$ is the set of observed region--policy pairs. For entries that are observed in the data, we retain the empirical estimate in \eqref{eq:obsgamma} (i.e.  $\hat{\gamma}_{R,i} = \gamma_{R,i}$); for entries that are unobserved, we impute $
    \hat{\gamma}_{R,i} = \hat{k}_R \hat{g}_i$. Then, for the hypothetical policy $\bm{P}=(\bm{I},\bm{T})$, we define $\gamma_R^P(t)$ as a piecewise-constant function of the region-specific policy effects:
\begin{equation}
...
{\color{blue}
We construct $\mathcal{I}$ from the Oxford COVID-19 Government Response Tracker (OxCGRT) using observations from 167 countries and regions. Each region-day is mapped into a mutually exclusive and collectively exhaustive policy category obtained by binarizing the underlying OxCGRT indicators and aggregating them into three components: restrictions on mass gatherings, school closures, and a composite “other” category capturing workplace closure, transport restrictions, and travel controls. Formally,
\[
\text{Restrict Mass Gatherings} = \mathbf{1}[\text{C3} \lor \text{C4} \lor \text{C5}],
\qquad
\text{Others} = \mathbf{1}[\text{C2} \lor \text{C7} \lor \text{C8}],
\]
with school closures (C1) and stay-at-home orders (C6) entering separately.

These components induce seven bundled policies ordered by increasing restrictiveness: (1) no measure; (2) mass gatherings only; (3) others only; (4) mass gatherings and schools; (5) mass gatherings and others; (6) mass gatherings, schools, and others; and (7) stay-at-home orders. Stay-at-home policies take precedence and define the most restrictive category. Table~\ref{tab:oxford_mapping} summarizes the mapping. In Appendix~\ref{app:granular}, we further show that all our qualitative conclusions hold under a finer 9-category partition that splits the two broadest bundles by within-category stringency.
...
Time is discretized at the monthly level over the horizon $[2020.03.15, 2020.06.15]$. A policy $\bm{P}$ is a sequence of three interventions from $\mathcal{I}$, one per month. We evaluate the full policy space $\mathcal{P} = \mathcal{I}^3$, yielding $7^3$ feasible policies. We refer to a policy by its sequence of category indices; for example, $(7,7,7)$ denotes a three-month lockdown.
...
\begin{table}[!htbp]\centering
	\caption{Within-Period Fatigue Effects}
	\label{tab:fatigue-effects}
	\begin{tabular}{lcccc}
		\toprule
		& (1) & (2) & (3) & (4) \\
		\midrule
		& Log review time & Mean review time & Synthetic sensitivity & Non-synthetic flagging \\
		Within-period order & -0.0086*** & -1.7472*** & 0.0104* & 0.0042 \\
		& (0.0011) & (0.3703) & (0.0050) & (0.0030) \\
		Low × order (vs control) & -0.0004 & -0.1612 &  & -0.0005 \\
		& (0.0010) & (0.2032) &  & (0.0012) \\
		Medium × order (vs control) & 0.0006 & 0.0280 &  & 0.0005 \\
		& (0.0009) & (0.2220) &  & (0.0008) \\
		High × order (vs control) & -0.0002 & -0.0899 &  & 0.0024* \\
		& (0.0009) & (0.2344) &  & (0.0012) \\
		Medium × order (vs low) &  &  & 0.0034 &  \\
		&  &  & (0.0030) &  \\
		High × order (vs low) &  &  & 0.0023 &  \\
		&  &  & (0.0033) &  \\
		\midrule
		Student FE & Yes & Yes & Yes & Yes \\
		Document FE & Yes & Yes & Yes & Yes \\
		Period FE & Yes & Yes & Yes & Yes \\
		Global order & Yes & Yes & Yes & Yes \\
		Pre-survey × order & Yes & Yes & Yes & Yes \\
		Observations & 8,117 & 8,117 & 8,525 & 129,193 \\
		\bottomrule
		\multicolumn{5}{p{0.95\linewidth}}{\footnotesize Note. The table reports within-period order slopes and condition-by-order interactions. Columns (1) and (2) are OLS; columns (3) and (4) are logit models. Standard errors are clustered by participant. † p < 0.10, * p < 0.05, ** p < 0.01, *** p < 0.001.}\\
	\end{tabular}
\end{table}
\end{landscape}
...
{\color{blue}
To ensure that THEMIS is usable as a framework for future pandemics, we further consider an online version of THEMIS that only utilizes the pandemic data sequentially as they enter. Specifically, we implement THEMIS in a rolling-horizon fashion in which, We therefore implement online THEMIS as a rolling-horizon version of the framework. At each monthly decision date, it uses the dated DELPHI snapshot available at that time, estimates policy effects from the data observed up to that date, and chooses the next-month NPI by one-step lookahead. 
\begin{table}[H]
...
Table~\ref{tab:rolling_advisor_v2} shows that online THEMIS improves on the realized policy cost in Brazil, Spain, and New York, reducing the mean gap to hindsight from $124.4\%$ to $39.5\%$. The selected sequences are also not uniformly draconian: after the first month, online THEMIS typically moves toward moderate restrictions rather than extended lockdown. We view this exercise as evidence that the framework can support sequential policy updating once early-wave information is available. We note that this analysis  assumes that the region-specific cost parameters in \cite{app:}. Of course, it is impossible  cost parameters for the  though the fully cold-start problem of a novel pathogen remains outside the scope of the paper.}
...
Second, confounding remains an inherent concern because NPIs are not randomly assigned. Voluntary behavior, seasonal factors, testing intensity, and other contemporaneous changes may all move together with policy. \rev{In Appendix~\ref{app:confounding}, we conduct a sensitivity analysis based on \cite{cinelli2020making} that shows our policy recommendations are highly stable even after a high degree of shrinkage towards a null benchmark. This sensitivity analysis do not eliminate the identification concerns of an observational study, but they do indicate that the paper's qualitative policy conclusions are not driven by a knife-edge estimate of the differential policy effects.}
...
In the following, we test the suitability of the rank-1 estimation against both higher-rank models and simpler baselines.
