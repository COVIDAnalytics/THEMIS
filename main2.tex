...
\usepackage{graphicx, amsmath, amssymb, amssymb, amsfonts, enumerate, booktabs, float, adjustbox, hyperref, tikz, multirow,pgfplots,wrapfig,bm,bbm,graphicx,subcaption,enumitem,makecell}
\usepackage{url}
...
\noindent\textbf{Problem Definition}:  Since December 2019, the COVID-19 pandemic has prompted governments worldwide to implement a wide range of non-pharmaceutical interventions (NPIs), from mass gathering restrictions to complete lockdowns. While these measures have been effective in reducing virus transmission, they also carry significant economic and humanitarian costs, including unemployment, mental health issues, and social unrest. This work aims to quantitatively assess the trade-offs between these diverse NPIs across economic and humanitarian dimensions, and to identify pandemic mitigation strategies tailored to different regions in the early stage of an outbreak.

\noindent\textbf{Methodology}: We develop THEMIS, a comprehensive data-driven counterfactual prediction framework that integrates econometric modeling, a policy-driven epidemiological model, and data-driven cost estimates. \rev{To recover policy effects when each region is observed under only a subset of NPIs, we estimate region and policy effects jointly under a rank-$1$ low-rank structure and evaluate that approximation through held-out entries, later-period transfer tests, and alternative policy partitions.} THEMIS quantifies the impacts of various NPIs across multiple dimensions during a pandemic, enabling a robust comparison of counterfactual scenarios.


\noindent\textbf{Results}: Leveraging data from 80+ sources, we apply THEMIS to evaluate thousands of policy alternatives across New York, Germany, Brazil, and Spain during COVID-19. Our analysis reveals that actual policies often deviated from the efficient frontier, leading to high costs in both humanitarian and economic dimensions. Short but moderately strong restrictions (e.g., limits on gatherings, travel, and work for 6–8 weeks) generally yielded the best outcomes, whereas prolonged lockdowns imposed high costs without proportional benefits. \rev{We also show that a rolling-horizon version of THEMIS, initialized with dated DELPHI snapshots and a warm-start approximation in the first month, would have improved on the realized policy cost in three of the four focal regions. In addition, a pooled region-blind decision tree provides an interpretable compression of the simulated policy map in per-capita and per-GDP units.} Trade-off profiles varied significantly across regions, emphasizing the importance of tailoring policies to local economic and epidemiological conditions rather than adopting uniform strategies.

\noindent\textbf{Managerial Implications}: THEMIS provides policymakers with a structured way to quantify and compare the trade-offs between different NPIs, enabling more informed decision-making in pandemic response planning. \rev{Beyond retrospective frontier analysis, the framework yields two operational summaries: an interpretable decision tree that compresses the simulated state-action map for the focal regions, and a rolling-horizon advisor that can be updated sequentially as new data arrive after an initial warm start.} By tailoring strategies to the specific socio-economic contexts of different regions, governments can compare candidate responses on a common economic and humanitarian basis.
}
}
...
{\color{blue}
The appendices show that these conclusions are not artifacts of one particular modeling choice. Appendix~\ref{app:granular} replaces the seven policy bundles with a finer 9-category partition and obtains similar qualitative predictions along the realized paths, although the New York comparison shows that the partition is not entirely innocuous. Appendix~\ref{app:time_transfer} shows that policy effects estimated in one pandemic period retain modest incremental predictive content over a constant-$\gamma$ benchmark in the next period. Appendix~\ref{app:prescriptive} then converts the full simulation output into two operational summaries: a region-blind state-action tree that compresses the focal-region policy map, and a rolling-horizon advisor that uses dated information plus an initial warm start. Together these extensions make the prescriptive content of THEMIS more explicit than a frontier plot alone.}

\section{Limitations}
\label{sec:discussion}
{\color{blue}
Although THEMIS yields actionable insights, it remains subject to several important limitations. First, because policy effects are learned from historically observed bundles, the framework cannot evaluate an NPI that has never been implemented anywhere in the underlying data. The counterfactual exercise is therefore strongest when it compares feasible policies within a menu that has already appeared in practice. The same point limits fully day-zero use for a completely novel pathogen: before even limited first-wave data accumulate, a policymaker would need additional inputs such as expert priors or analogies to prior outbreaks. The online advisor in Appendix~\ref{app:prescriptive} shows that THEMIS can be deployed sequentially once early-wave information begins to arrive, but it is not a zero-data oracle.

Second, confounding remains an inherent concern because NPIs are not randomly assigned. Voluntary behavior, seasonal factors, testing intensity, and other contemporaneous changes may all move together with policy. The revised appendix therefore focuses on the object most relevant for our conclusions: the ranking of candidate policies. Appendix~\ref{app:confounding} shows that the policy ordering is highly stable when the estimated differential NPI effects are shrunk substantially toward a no-contrast benchmark, and supplements this with an event-study analysis around 716 policy transitions. These exercises do not eliminate the usual identification concerns of an observational study, but they do indicate that the paper's qualitative policy conclusions are not driven by a knife-edge estimate of the differential policy effects.

Third, the component models necessarily simplify a complex phenomenon. DELPHI, like other SEIR-based models, requires a sufficiently large population and epidemic size for its aggregate dynamics to be reliable \citepmain{holmdahl2020}. The cost model is likewise intended as a first-order accounting framework rather than a complete welfare representation. Some channels are modeled linearly or additively for tractability. Appendix~\ref{app:additional_sensitivity} therefore revisits several of these assumptions directly: it shows that the policy ranking is essentially unchanged under nonlinear mental-health cost functions, that the epidemiological ranking is highly stable under within-block adherence decay, and that a shorter one-month optimization horizon tends to recommend a weaker first-month intervention than the baseline three-month problem. These checks reduce, but do not eliminate, model dependence.
...
\rev{The additional robustness exercises strengthen this conclusion materially. The rank-$1$ low-rank structure remains competitive across holdout exercises, higher-rank alternatives, finer policy partitions, and time-transfer tests. The prescriptive extensions likewise show that the framework can be distilled into operational guidance: the pooled state-action tree identifies threshold-based escalation and exit rules in observable units, while the online rolling-horizon advisor improves on the realized policy cost in three of the four focal regions using only information available at each decision date.}
...
\section{\rev{Prescriptive Extensions}}
\label{app:prescriptive}

{\color{blue}
\subsection*{Region-Blind State-Action Decision Tree}

To distill the full simulation output into an interpretable summary, we construct a pooled state-action dataset over the four focal regions, all policy prefixes of length $0$, $1$, and $2$, and a 20-point grid of humanitarian weights $w \in [0,1]$. For each tuple we record the observable state at the start of the next month and the cost-minimizing next-month NPI under full THEMIS simulation. The state vector includes cumulative cases, cumulative deaths, active cases, active hospitalizations, cumulative economic cost, cumulative humanitarian cost, and the severity of the realized policy prefix; it deliberately excludes any region indicator or region-specific covariate. Pooling across the four regions yields $4{,}560$ (state, action) observations.

We then train a single pooled CART tree of depth 4 with minimum leaf size 20, using $w$ as an additional feature. The resulting tree has five-fold cross-validation accuracy 0.47 and produces 16 leaves. Table~\ref{tab:state_action_action_distribution} summarizes how the optimal action shifts across the weight grid. We view the tree as a compression of the simulated state-action map for the focal regions rather than as a separately validated predictor for new regions.
}
...
{\color{blue}
The pooled tree is shown in Figure~\ref{fig:state_action_pooled_tree}. The most managerially informative leaves are summarized in Table~\ref{tab:state_action_rules}. Four findings stand out. First, active epidemiological burden---especially active cases and active hospitalizations---drives the key escalation and exit splits. Second, once cumulative economic cost becomes large and active hospitalizations fall below a small threshold, the tree prescribes rapid relaxation. Third, lockdown appears only as an early, preemptive tool; later in the horizon the tree prefers NPI~6 over NPI~7. Fourth, even economy-first planners face states in which rising deaths trigger renewed restriction.
}
...
\caption{\rev{Five managerially informative leaves of the pooled state-action tree (depth 4, minimum leaf size 20, 4,560 training points). None of the conditions references a region indicator.}}
...
{\color{blue}
\subsection*{True Online Rolling-Horizon Advisor}

We also implement a rolling-horizon version of THEMIS with an explicit warm start. At each monthly decision date $t \in \{$Mar~15, Apr~15, May~15$\}$, the advisor uses dated DELPHI snapshots and date-restricted policy information available by that point to choose the next-month NPI by one-step lookahead. When the first decision window is too short for the date-restricted matrix estimate, we use the factory's rank-$1$ gamma estimate as a warm start and then switch to the date-restricted estimator from the second decision onward.
}
...
\caption{\rev{Online rolling-horizon advisor versus the unattainable hindsight optimum. Costs are expressed as \% of quarterly GDP. The gap columns report percentage distance from the hindsight-optimal cost.}}
...
\caption{\rev{Three-month total cost for each region under the online advisor, the realized historical policy, and the hindsight optimum. Bars decompose total cost into economic and humanitarian components.}}
\label{fig:rolling_advisor_v2}
\end{figure}

{\color{blue}
The online advisor outperforms the realized historical policy in Brazil, Spain, and New York, and reduces the mean gap to hindsight from 124.4\% to 39.5\%. The sequences it selects are also not uniformly draconian: after the initial month, the advisor typically moves toward moderate restrictions rather than prolonged lockdown.
...
\]
Evaluating the hindsight policy, the online advisor, and the realized policy under this common $w_R^\star$ provides a more charitable comparison from the perspective of the policymaker's own revealed trade-off.
}
...
{\color{blue}
The revealed-preference analysis sharpens the same message. Germany's actual policy is comparatively close to its own hindsight optimum, whereas Spain and New York remain far from optimal even after one evaluates them under the most favorable weight. The online advisor reduces weighted cost relative to the realized policy in all four regions, with especially large gains in Spain and New York.}

\section{\rev{Additional Sensitivity Analyses}}
\label{app:additional_sensitivity}

{\color{blue}
\subsection*{Nonlinear Mental-Health Cost Functions}
...
\subsection*{Within-Block Adherence Decay}
...
\subsection*{Planning Horizon: One Month versus Three Months}
