...
We train a single pooled CART tree ($\max\!\text{depth}=4$, $\min\!\text{leaf}=20$) that takes $w$ as an additional feature, with five-fold cross-validation accuracy $0.56$. The action distribution alone (Table~\ref{tab:state_action_action_distribution}) is already informative: as $w$ increases the optimal next-month NPI shifts monotonically from the relaxed end of the menu at $w=0$ to the strict end at $w=1$, and \emph{the partial-restriction NPIs~$3$ and $4$ are essentially never optimal} ($9$ and $1$ rows out of $1{,}140$), so the seven-NPI menu effectively collapses to a five-NPI menu in which lifting mass-gathering restrictions while travel/work restrictions remain in place is strictly dominated.
...
$0.25$ & $80$  & $0$ & $4$ & $0$ & $67$ & $59$  & $18$ \\
$0.50$ & $55$  & $0$ & $1$ & $0$ & $55$ & $91$  & $26$ \\
$0.75$ & $36$  & $0$ & $0$ & $0$ & $38$ & $117$ & $37$ \\
$1.00$ & $3$   & $5$ & $0$ & $1$ & $2$  & $162$ & $55$ \\\hline
\end{tabular}
\caption{Distribution of the cost-optimal next-month NPI across $228$ (region, prefix) decision points per row, broken down by humanitarian weight $w$. The optimum shifts monotonically from $\{1,5\}$ at $w=0$ to $\{6,7\}$ at $w=1$. NPIs~$3$ (lifting mass-gathering restrictions while keeping travel and work restricted) and $4$ (mass-gathering plus school restrictions only) almost never appear; both are strictly Pareto-dominated alternatives on the simulated grid.}
\label{tab:state_action_action_distribution}
\end{table}

The pooled tree is reported in Figure~\ref{fig:state_action_pooled_tree} and produces $15$ leaves, all with $n \geq 20$ training points by construction. We highlight the five leaves with the most managerial content in Table~\ref{tab:state_action_rules}, sorted by purity.
...
R1 & $w \leq 0.375$,\; cum.\ economic cost $> 14.5\%$ of monthly GDP,\; cum.\ cases $\leq 0.57\%$ of population,\; mean prefix severity $\leq 5.25$ & $1$ & $100\%$ & $80$ \\
R2 & $w > 0.625$,\; cum.\ economic cost $> 6.57\%$ of monthly GDP,\; cum.\ cases $> 0.87\%$ of population & $6$ & $98\%$ & $142$ \\
R3 & $w \leq 0.375$,\; cum.\ economic cost $> 14.5\%$ of monthly GDP,\; cum.\ cases $> 0.57\%$ of population,\; cum.\ humanitarian cost $> 65.5\%$ of monthly GDP & $1$ & $85\%$ & $46$ \\
R4 & $w > 0.375$,\; cum.\ economic cost $\leq 6.57\%$ of monthly GDP,\; active hospitalisations $\leq 0.018\%$ of population,\; cum.\ deaths $> 0.001\%$ of population & $7$ & $79\%$ & $111$ \\
R5 & $0.125 < w \leq 0.375$,\; cum.\ economic cost $\leq 4.61\%$ of monthly GDP & $7$ & $41\%$ & $44$ \\\hline
\end{tabular}
\caption{Five managerially informative leaves of the pooled state-action tree (depth~$4$, minimum leaf size~$20$, $1{,}140$ training points). None of the conditions references a region indicator or a region-specific covariate. R1--R4 jointly cover $379$ of the $1{,}140$ training points and characterise the modal exit, high-$w$ wave-management, and preemptive-lockdown regimes; R5 has lower purity but identifies the only sub-population in which a low-$w$ policymaker is told to lock down.}
...
\caption{Pooled state-action decision tree with humanitarian weight $w$ as a feature (depth~$4$, minimum leaf size~$20$, five-fold CV accuracy~$0.56$). Internal nodes split on the seven observable state features and on $w$; leaves predict the optimal next-month NPI, colour-coded by class. All $15$ leaves have at least $20$ training points.}
...
Four substantive insights follow. First, \emph{cumulative cost variables, not the policy prefix or the public-health snapshot, are the dominant non-$w$ splits}. The first non-$w$ split in every branch of the tree is on \texttt{cum\_econ\_cost\_pct\_gdp}; the second-level splits add \texttt{pop\_pct\_cum\_cases} or \texttt{cum\_human\_cost\_pct\_gdp}. Mean prefix severity and active hospitalisations only appear at depth~$4$. The optimal-action map is therefore best read in cost-budget space, not in policy-history space.

Second, low-$w$ exit timing is sharp and observable. Rule~R1 covers $80$ pooled decision points with $100\%$ purity and gives a one-line trigger: an economy-prioritising policymaker ($w \leq 0.375$) who has spent more than $14.5\%$ of monthly GDP on cumulative economic cost, while the wave has not yet broken ($\leq 0.57\%$ of the population infected) and the prefix has been moderate ($\bar s_{1:t-1} \leq 5.25$), should release immediately to NPI~$1$. The rule is observable from running tallies and does not require any forward simulation.

Third, \emph{lockdown is rare}. Of the $15$ pooled leaves, only two majority-vote for lockdown with non-trivial purity: R4 ($79\%$, $n=111$) and R5 ($41\%$, $n=44$). Lockdown is never optimal once cumulative economic cost crosses $6.57\%$ of monthly GDP. The reason is that, in our calibrated cost model, lockdown's incremental epidemiological gain over the next-strictest bundle (NPI~$6$) shrinks once mental-health damage has compounded, while its incremental economic cost continues to grow; the tree internalises this trade-off without being told to. Rule~R2, the modal high-$w$ leaf, recommends NPI~$6$ rather than lockdown for $98\%$ of $142$ decision points whenever $w > 0.625$ and the wave is well underway.

Fourth, two rules run against the obvious ``high $w \Rightarrow$ stricter NPI, low $w \Rightarrow$ looser NPI'' heuristic. R3 ($85\%$ pure, $n=46$) shows that even after a humanitarian catastrophe---cumulative mortality cost above $65\%$ of monthly GDP and the wave already past $0.57\%$ infected---a low-$w$ policymaker who has hit the $14.5\%$ economic-cost cap should still release to NPI~$1$: the sunk economic damage is irreversible and further restriction merely compounds it. Conversely, R4 prescribes lockdown precisely when the economic budget is still healthy ($\leq 6.57\%$ of GDP), active hospitalisations are still low, but cumulative deaths have begun to register ($> 0.001\%$ of population)---a preemptive escalation that triggers on early mortality signals rather than on hospital strain. Both rules contradict the stylised view that low $w$ implies looseness and that lockdown is a late-stage tool.
...
\hline
\textbf{Region} & $\pi^{\text{online}}_R$ & $\pi^{\text{tree}}_R$ & $w^\star$ & $\pi^{\text{actual}}_R$ & $\pi^{\text{HS}}_R$ & \textbf{Online} & \textbf{Tree} & \textbf{Actual} & \textbf{Hindsight} \\
                &                          &                       &           &                         &                     & \textbf{cost}   & \textbf{cost} & \textbf{cost}   & \textbf{cost}      \\\hline
Germany   & $7$--$4$--$5$ & $6$--$6$--$6$ & $1.00$ & $6$--$6$--$5$ & $6$--$6$--$1$ & \$$277.4$B   & \$$79.0$B    & \$$77.0$B    & \$$60.8$B   \\
Brazil    & $4$--$4$--$7$ & $7$--$5$--$5$ & $0.25$ & $3$--$3$--$3$ & $6$--$6$--$5$ & \$$147.0$B   & \$$92.4$B    & \$$239.0$B   & \$$88.8$B   \\
Spain     & $7$--$4$--$5$ & $6$--$1$--$1$ & $0.50$ & $7$--$7$--$6$ & $6$--$1$--$1$ & EUR\,$75.6$B & EUR\,$35.8$B & EUR\,$97.5$B & EUR\,$35.8$B \\
New York  & $7$--$4$--$1$ & $6$--$6$--$6$ & $1.00$ & $7$--$7$--$6$ & $6$--$6$--$1$ & \$$149.7$B   & \$$127.7$B   & \$$264.9$B   & \$$116.6$B  \\\hline
\multicolumn{6}{|r|}{\textbf{Mean cost gap to hindsight}} & \textbf{$+140.1\%$} & \textbf{$+10.9\%$} & \textbf{$+123.6\%$} & \textbf{$0\%$} \\\hline
\end{tabular}
\caption{Three feasible online policies versus the unattainable hindsight optimum. $\pi^{\text{online}}_R$: rolling-horizon advisor with one-step lookahead under online rank-$1$ ALS gammas. $\pi^{\text{tree}}_R$: pooled state-action tree of \S(A) queried month-by-month with the THEMIS-induced state of its own prefix and the prescriber's preference weight $w^\star$. The tree is trained once offline; for each region we report the cost-minimising prescription over $w \in \{0, 0.25, 0.5, 0.75, 1\}$, with the selected $w^\star$ shown in the fourth column---this is a feasible online policy provided the prescriber commits to $w^\star$ ex ante. $\pi^{\text{actual}}_R$: the policy actually implemented; the cost is the real observed cost via \texttt{policy\_type="actual"}. $\pi^{\text{HS}}_R$: hindsight-optimal sequence over all $7^3=343$ enumerations under full-window $\hat\gamma^{\text{full}}$. Costs are in local currency (\$B for DE, BR, US-NY; EUR\,B for ES). NPI numbering follows the main paper. Online and tree-policy costs are evaluated under $\hat\gamma^{\text{full}}$ on a single ground truth so the comparison isolates the policy-choice effect.}
...
\caption{Three-month total cost for each region under the online advisor, the cost-minimising tree policy, the actual implemented policy, and the hindsight optimum. Bars decompose total cost into economic (blue) and humanitarian (red) components. Sequences below each bar follow the policy numbering of the main paper.}
\label{fig:rolling_advisor_v2}
\end{figure}

The pooled tree of \S(A) is the strongest of the three feasible policies. Averaged across regions, the residual gap to the hindsight optimum is $10.9\%$ for the tree policy, $123.6\%$ for the actual policy, and $140.1\%$ for the rolling-horizon advisor; the tree dominates both the actual policy and the rolling-horizon advisor in every region, and matches the hindsight optimum exactly in Spain. Because the tree is trained once offline and queried at the prescriber's preference weight $w$ as a parameter, $w^\star$ in the fourth column should be read as ``the trade-off setting that this region's optimal-cost record turned out to favour''; the dispersion of $w^\star$ across regions reflects genuine differences in the underlying frontier (humanitarian-leaning settings dominate in Germany and New York, where wave magnitudes were severe; balanced settings dominate in Spain; an economy-leaning setting dominates in Brazil, where the actual humanitarian cost was already very high under the implemented loose policy).

The rolling-horizon advisor of \S(B) is the weaker policy on this sample and underscores the cost of one-step lookahead in a region where early estimation is noisy. Its worst region is Germany, where the Mar~15 decision is taken on an essentially empty policy window: the snapshot-based zmean fallback estimator yields a lockdown gamma of $0.239$, the advisor consequently recommends lockdown for the first month, and that early lockdown is then evaluated against a hindsight ground truth in which the marginal benefit of lockdown over NPI~$6$ is small. By Apr~15 the cross-region ALS estimator has produced a discriminating gamma vector and the advisor switches to NPI~$4$ then NPI~$5$, but the cost of the first-month over-escalation has already dominated the trajectory. In Brazil, Spain, and New York the rolling-horizon advisor still beats the actual policy by a wide margin (\$$92$B in Brazil, EUR\,$22$B in Spain, \$$115$B in New York). The contrast with the tree policy makes a clean managerial point: when a portable, cost-tested rule is available, it should be preferred; when only the rolling-horizon advisor can be deployed, it still substantially outperforms what was actually implemented in three of four regions.
